...
import { localRequestAccess } from "./localAccess";
import { serveStudioFile } from "./studioFiles";
import { z } from "zod";
import {
...
import { DesignService, designResponseStatus, type GenerationCommandReceipt } from "./designService";
import { createDesignMedia } from "./designMedia";
import { DesignError, reconcilePersistentJobs } from "@petlord/design-core";
import { characterProjectSchema, identityProfileSchema, type CharacterProject } from "@petlord/schema";
import { assertGenerationBudget } from "./generationBudget";
import { estimateProviderRequestCost } from "./providers/cost";
import { createInstalledPackageImporter, type InstalledPackageHostSummary, type InstalledSyncResult } from "./installedPackages";

export interface InstalledPackageInstallOptions { mode: "new" | "replace"; targetKey?: string }

export interface PetLordServerOptions {
  host?: string;
  port?: number;
  runtimeDataDirectory?: string;
  studioDirectory?: string;
  installedPackagesDirectory?: string;
  authToken?: string;
  ffmpegPath?: string;
  foregroundMaskerPath?: string;
  installPackage?: (contents: Uint8Array, options: InstalledPackageInstallOptions) => Promise<unknown>;
  getInstalledPackageStatus?: (key: string) => Promise<InstalledPackageHostSummary | undefined>;
}

export async function startPetLordServer(options: PetLordServerOptions = {}): Promise<{ url: string; close(): Promise<void>; syncInstalledPackages():Promise<InstalledSyncResult>; getInstalledPackageBindings():Promise<Array<{packageKey:string;projectId:string;appliedRevision?:number}>> }> {
  const port = options.port ?? Number(process.env.PETLORD_API_PORT ?? 4312);
  const host = options.host ?? process.env.PETLORD_API_HOST ?? "127.0.0.1";
  if (!["127.0.0.1", "localhost", "::1"].includes(host)) throw new Error("PetLord design service must bind to a loopback interface.");
  const ffmpegPath: string | null = options.ffmpegPath ?? process.env.PETLORD_FFMPEG_PATH ?? createRequire(import.meta.url)("ffmpeg-static");
  let serviceUrl = "";
  let closing = false;
  ...
  const runtimeDataDirectory = resolve(options.runtimeDataDirectory ?? process.env.PETLORD_RUNTIME_DATA_DIR ?? defaultRuntimeDataDirectory);
  ...
  const foregroundMaskerBinary = options.foregroundMaskerPath ?? join(nativeDirectory, "foreground-masker");
  ...
  const providerRegistry = new GenerationProviderRegistry(sqliteStore, lifetime.signal);
  ...
  const execFileAsync = lifetime.execute;
  ...
  const workspaceEntityEnvelopeSchema = z.object({ data: z.unknown(), expectedRevision: z.number().int().nonnegative().nullable().optional() });
  ...
  async function cacheRemoteMedia(sourceUrl: string) {
    ...
    const remote = await fetch(sourceUrl, { signal: AbortSignal.any([lifetime.signal, AbortSignal.timeout(120_000)]) });
  ...
  async function saveDataUrl(dataUrl: string, suppliedMediaId?: string) {
    ...
    await mkdir(mediaDirectory, { recursive: true });
    const mediaId = suppliedMediaId ?? randomUUID();
  ...
  async function ensureForegroundMasker() {
    ...
    if (options.foregroundMaskerPath) {
      await stat(options.foregroundMaskerPath);
      return options.foregroundMaskerPath;
    }
  ...
  function publicJob(job: StoredJob): PersistentGenerationJob {
    const { providerCapability: _providerCapability, providerRequest: _providerRequest, postprocess: _postprocess, ...visible } = job;
    return visible;
  }
  ...
  function reconcileProjectJobs(projectId: string) {
    const snapshot = sqliteStore.getEntitySnapshot<CharacterProject>("project", projectId);
    if (!snapshot) return;
    const parsed = characterProjectSchema.safeParse(snapshot.data);
    if (!parsed.success) return;
    const next = reconcilePersistentJobs(parsed.data, [...jobs.values()].map(publicJob));
    if (next !== parsed.data) sqliteStore.commitEntityCommand({ type: "project", id: projectId, expectedRevision: snapshot.revision, data: next }, () => undefined);
  }
  ...
  async function updateStoredJob(job: StoredJob, patch: Partial<StoredJob>) {
    ...
    jobs.set(job.id, job);
    await persistJobs();
    reconcileProjectJobs(job.trigger.projectId);
  }

  async function runStoredJob(job: StoredJob) {
    if (closing || runningJobs.has(job.id) || ["succeeded", "failed"].includes(job.status)) return;
    ...
    try {
      ...
      if (provider.capability === "image") {
        ...
        await updateStoredJob(job, {
          ...
          cost: (() => {
            const models = providerRegistry.getPublic(provider.id)?.models;
            if (models?.find(model => model.id === job.model)?.estimatedUnitCostCny !== undefined) return job.cost;
      ...
      for (; !closing;) {
        ...
        if (task.status === "succeeded") {
          ...
          if (job.kind === "state-image") {
            ...
            await updateStoredJob(job, {
              ...
              cost: (() => {
                if (!job.cost || providerRegistry.getPublic(provider.id)?.models.find(model => model.id === job.model)?.estimatedUnitCostCny !== undefined) return job.cost;
        ...
        await delay(8000, undefined, { signal: lifetime.signal });
      }
    } catch (error) {
      await updateStoredJob(job, closing && job.remoteTaskId
        ? { status: "queued", error: undefined }
        : { status: "failed", error: closing ? "服务在提交或处理期间关闭。Provider 可能已受理，为避免重复计费未自动重提；检查任务和账单后再手动重试。" : error instanceof Error ? error.message : "生成任务失败" });
    } finally {
  ...
  function scheduleStoredJobs() {
    ...
    for (const job of pending) {
      const running = runStoredJob(job);
      activeJobs.add(running);
      void running.then(() => activeJobs.delete(running), () => activeJobs.delete(running));
    }
  }

  async function submitStoredJob(rawInput: unknown, receipt?: GenerationCommandReceipt): Promise<PersistentGenerationJob> {
    if (closing) throw new DesignError("UNAVAILABLE", "PetLord service is stopping.");
    const input = jobSubmissionSchema.parse(rawInput);
    const existing = jobs.get(input.id);
    if (existing) {
      if (receipt && !sqliteStore.getDesignReceipt(receipt.requestId)) throw new DesignError("REQUEST_ID_REUSED", "This request id already identifies a different persisted job.");
      return publicJob(existing);
    }
    if ((input.kind === "state-image") !== (input.capability === "image")) {
      throw new DesignError("INVALID_INPUT", "Generation job kind does not match provider capability.");
    }
    ...
    if ((providerRequest as { model: string }).model !== input.model) throw new DesignError("INVALID_INPUT", "Job model does not match the provider request model.");
    const cost = estimateProviderRequestCost(providerRegistry.getPublic(provider.id)!, providerRequest);
    if (!cost) throw new DesignError("PRICE_UNAVAILABLE", "请在模型服务中补充此模型的预估费用，再生成。");
    const owningProject = sqliteStore.getEntitySnapshot<CharacterProject>("project", input.trigger.projectId)?.data;
    if (owningProject) assertGenerationBudget(owningProject, [...jobs.values()], cost?.estimatedMaxCny);
    ...
    if (receipt) {
      let queued = false;
      const result = sqliteStore.commitDesignOperation(receipt.requestId, receipt.requestHash, () => {
        const current = sqliteStore.getEntitySnapshot("project", receipt.projectId);
        if (current?.revision !== receipt.expectedRevision) throw new DesignError("REVISION_CONFLICT", "Project changed before generation could be queued.");
        sqliteStore.upsertGenerationJob(job);
        queued = true;
        return { protocolVersion: 1 as const, requestId: receipt.requestId, result: { job: publicJob(job) } };
      });
      if (!queued) return result.result.job;
      jobs.set(job.id, job);
    } else {
      jobs.set(job.id, job);
      await persistJobs();
  ...
  async function retryStoredJob(jobId: string, receipt?: Pick<GenerationCommandReceipt, "requestId" | "requestHash">): Promise<PersistentGenerationJob> {
    if (closing) throw new DesignError("UNAVAILABLE", "PetLord service is stopping.");
    const job = jobs.get(jobId);
    if (!job) throw new DesignError("NOT_FOUND", "Generation job not found.");
    if (job.status !== "failed") throw new DesignError("INVALID_INPUT", "Only failed generation jobs can be retried.");
    if (!job.remoteTaskId && !job.providerRequest) throw new DesignError("INVALID_INPUT", "Provider state is no longer available for safe retry.");
    const project = sqliteStore.getEntitySnapshot<CharacterProject>("project", job.trigger.projectId)?.data;
    if (!project) throw new DesignError("NOT_FOUND", "The job's project no longer exists.");
    const runtime = job.providerId ? providerRegistry.runtime(job.providerId) : providerRegistry.resolve(job.providerCapability ?? (job.kind === "state-image" ? "image" : "video"));
    const validatedRequest = job.remoteTaskId ? undefined : runtime.validateRequest(job.providerRequest);
    const estimate = job.remoteTaskId ? job.cost : estimateProviderRequestCost(providerRegistry.getPublic(runtime.id)!, validatedRequest);
    assertGenerationBudget(project, [...jobs.values()], job.remoteTaskId ? 0 : estimate?.estimatedMaxCny);
    const cost = estimate && !job.remoteTaskId ? { ...estimate, priorAttemptsReservedCny: (job.cost?.priorAttemptsReservedCny ?? 0) + (job.cost?.actualCny ?? job.cost?.estimatedMaxCny ?? 0) } : job.cost;
    const next = { ...job, status: "queued" as const, progress: job.remoteTaskId ? 12 : 0, error: undefined, cost, updatedAt: new Date().toISOString() };
    if (receipt) sqliteStore.commitDesignOperation(receipt.requestId, receipt.requestHash, () => {
      sqliteStore.upsertGenerationJob(next);
      return { protocolVersion: 1, requestId: receipt.requestId, result: { job: publicJob(next) } };
    }); else sqliteStore.upsertGenerationJob(next);
    jobs.set(job.id, next);
    reconcileProjectJobs(next.trigger.projectId);
    scheduleStoredJobs();
    return publicJob(next);
  }

  const designMedia = createDesignMedia(mediaDirectory, options.studioDirectory ?? fileURLToPath(new URL("../../studio/public/", import.meta.url)), publishedPackageLibrary, lifetime.signal);
  const installedPackages = createInstalledPackageImporter(sqliteStore, options.installedPackagesDirectory, saveDataUrl);
  const getInstalledPackageStatus = options.getInstalledPackageStatus;
  const designService = new DesignService(sqliteStore, {
    providers: providerRegistry,
    importMedia: saveDataUrl,
    resolveImage: designMedia.resolveImage,
    listJobs: () => [...jobs.values()].map(publicJob),
    submitJob: submitStoredJob,
    retryJob: retryStoredJob,
    exportPackage: designMedia.exportPackage,
    syncInstalledPackages: () => installedPackages.sync(),
    getPackageInstallation: async (projectId, currentRevision) => installedPackages.status(
      projectId,
      currentRevision,
      Boolean(options.installPackage && options.installedPackagesDirectory),
      getInstalledPackageStatus,
    ),
    processMedia: async (input, video, tail) => {
      if (input.operation === "ping-pong") return createPingPongMedia(pingPongMediaSchema.parse({ ...input, videoUri: video.uri, hasAlpha: video.hasAlpha ?? false, pixelWidth: video.pixelWidth, pixelHeight: video.pixelHeight }));
      if (input.operation === "extract-frame") {
        if (!ffmpegPath) throw new Error("FFmpeg runtime is unavailable.");
        const filename = `${randomUUID()}.png`;
        await execFileAsync(ffmpegPath, ["-y", ...(video.hasAlpha ? ["-c:v", "libvpx-vp9"] : []), "-i", localMediaPath(video.uri), "-ss", (input.timeMs / 1000).toFixed(3), "-frames:v", "1", "-vf", "format=rgba", join(mediaDirectory, filename)], { timeout: 120_000, maxBuffer: 4 * 1024 * 1024 });
        const bytes = await readFile(join(mediaDirectory, filename));
        return { tail: { uri: `/api/media/${filename}`, mimeType: "image/png", pixelWidth: bytes.readUInt32BE(16), pixelHeight: bytes.readUInt32BE(20), hasAlpha: video.hasAlpha ?? false, frameTimeMs: input.timeMs } };
      }
      const parsed = transparentizeMediaSchema.parse({ ...input, videoUri: video.uri, tailUri: tail?.uri });
      const processed = await normalizeGeneratedVideo(parsed.videoUri, parsed.tailUri, parsed.resolution, true, parsed.keyColor, parsed.similarity);
      const common = { provider: "local-ffmpeg", model: processed.transparencyMethod, pixelWidth: processed.pixels, pixelHeight: processed.pixels, hasAlpha: true, transparencyMethod: processed.transparencyMethod === "none" ? undefined : processed.transparencyMethod, alphaCoverage: processed.coverage };
      return { video: { ...common, uri: processed.videoUri, mimeType: processed.videoMimeType, silent: true }, tail: { ...common, uri: processed.tailUri, mimeType: "image/png" } };
    },
    installPackage: options.installPackage ? async (project, input) => {
      const exported = await designMedia.exportPackage(project);
      const encoded = await publishedPackageLibrary.read(exported.publicationId);
      if (!options.installedPackagesDirectory) {
        const installed = await options.installPackage!(encoded, { mode: "new" });
        const binding = await installedPackages.status(input.projectId,input.revision,false,getInstalledPackageStatus);
        return { package: exported, installed, binding };
      }
      const applied = await installedPackages.applyInstalledPackage(
        input.projectId,
        input.revision,
        encoded,
        getInstalledPackageStatus,
        async before => {
          if (before.linked && !before.exists) throw new DesignError("NOT_FOUND", "The linked installed pet no longer exists. Open the pet list and choose it again.");
          const installOptions: InstalledPackageInstallOptions = before.linked
            ? { mode: "replace", targetKey: before.packageKey }
            : { mode: "new" };
          const installed = await options.installPackage!(encoded, installOptions);
          const packageKey = installed && typeof installed === "object" && "key" in installed && typeof installed.key === "string"
            ? installed.key
            : undefined;
          if (!packageKey) throw new Error("The desktop host did not return the installed package key.");
          return { installed, packageKey };
        },
      );
      return { package: exported, installed: applied.installed, binding: applied.binding };
    } : undefined,
  });

  const server = createServer(async (request, response) => {
    if (closing) return writeJson(response, 503, { error: { code: "UNAVAILABLE", message: "PetLord service is stopping." } });
    const denied = localRequestAccess(request.headers, serviceUrl, options.authToken);
    if (denied) return writeJson(response, denied.status, { error: { code: denied.code, message: denied.message } });
    const url = new URL(request.url ?? "/", serviceUrl);
    ...
    try {
      if (request.method === "GET" && url.pathname === "/api/design/v1/commands") return writeJson(response, 200, designService.catalog());
      if (request.method === "POST" && url.pathname === "/api/design/v1/execute") {
        const result = await designService.execute(await readJson(request));
        return writeJson(response, designResponseStatus(result), result);
      }
      if (request.method === "GET" && url.pathname === "/api/health") return writeJson(response, 200, { status: "ready", protocolVersion: 1 });
      ...
      if (entityMatch) {
        ...
        if (request.method === "GET" && !id) return writeJson(response, 200, url.searchParams.get("versioned") === "1" ? sqliteStore.listEntitySnapshots(type) : sqliteStore.listEntities(type));
        if (request.method === "GET" && id) {
          const snapshot = sqliteStore.getEntitySnapshot(type, id);
          return snapshot ? writeJson(response, 200, snapshot) : writeJson(response, 404, { error: { code: "NOT_FOUND", message: "Workspace record not found." } });
        }
        if (request.method === "PUT" && id) {
          ...
          if (input.expectedRevision === undefined) throw new DesignError("REVISION_REQUIRED", "A workspace revision is required. Reload the Studio before saving.");
          const data = type === "project" ? reconcilePersistentJobs(characterProjectSchema.parse(input.data), [...jobs.values()].map(publicJob))
            : type === "identity" ? identityProfileSchema.parse(input.data) : input.data;
          const saved = sqliteStore.commitEntityCommand({ type, id, data, expectedRevision: input.expectedRevision }, snapshot => snapshot);
          return writeJson(response, 200, saved);
        }
        if (request.method === "DELETE" && id) {
          const expected = url.searchParams.get("expectedRevision");
          if (expected === null || !/^\d+$/.test(expected)) throw new DesignError("REVISION_REQUIRED", "Read the record before deleting it.");
          const deleted = sqliteStore.commitEntityCommand({ type, id, data: null, expectedRevision: Number(expected) }, snapshot => ({ deleted: true, revision: snapshot.revision }));
          return writeJson(response, 200, deleted);
        }
      }
      ...
      if (providerMatch) {
        ...
        if (request.method === "PUT") {
          ...
          if ([...jobs.values()].some(job => job.providerId === id && !["succeeded", "failed"].includes(job.status))) {
      ...
      if (request.method === "POST" && retryJobMatch) {
        ...
        return writeJson(response, 202, await retryStoredJob(jobId));
      }
      ...
      if (request.method === "POST" && url.pathname === "/api/jobs") {
        return writeJson(response, 202, await submitStoredJob(await readJson(request)));
      }
      ...
      if (options.studioDirectory && !url.pathname.startsWith("/api/") && await serveStudioFile(request, response, options.studioDirectory, url.pathname)) return;
      return writeJson(response, 404, { error: { message: "Not found." } });
    } catch (error) {
      if (error instanceof DesignError) return writeJson(response, ["REVISION_CONFLICT", "REVISION_REQUIRED", "REFERENCED"].includes(error.code) ? 409 : 400, { error: { code: error.code, message: error.message, details: error.details } });
      if (error instanceof SyntaxError) return writeJson(response, 400, { error: { code: "INVALID_JSON", message: "Request body must be valid JSON." } });
  ...
  await loadJobs();
  await installedPackages.sync();
  for (const projectId of new Set([...jobs.values()].map(job => job.trigger.projectId))) reconcileProjectJobs(projectId);
  ...
  } catch (error) { closing = true; await lifetime.close(); sqliteStore.close(); throw error; }
  const address = server.address();
  if (!address || typeof address === "string") throw new Error("Local server did not allocate a TCP port.");
  serviceUrl = `http://${host === "::1" ? "[::1]" : host}:${address.port}`;
  scheduleStoredJobs();
  let closePromise: Promise<void> | undefined;
  return {
    url: serviceUrl,
    syncInstalledPackages() { if(closing) return Promise.reject(new Error("设计服务正在关闭。"));return installedPackages.sync(); },
    async getInstalledPackageBindings() { if(closing) throw new Error("设计服务正在关闭。");return installedPackages.bindings(); },
    close() {
      return closePromise ??= (async () => {
        ...
        await installedPackages.settle();
        sqliteStore.close();
      })();
    },
  };
