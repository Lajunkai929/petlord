...
it.each(["openai-compatible", "siliconflow", "volcengine-ark"] as const)("generates a headless %s custom model with imported references and preserves metadata across restart", async type => {
  ...
  try {
    ...
    expect(candidates.artifacts).toHaveLength(1); expect(candidates.artifacts[0]).toMatchObject({ pixelWidth: 1024, pixelHeight: 1024 });
    ...
    await api.close(); api = await startPetLordServer({ port: 0, runtimeDataDirectory: directory });
