...
const transitions: Transition[] = [
  ...
  transition({
    ...
    prompt: "Lottery 被双击肚皮后开心地从趴姿向侧面翻滚，再平稳翻到背上露出肚皮，四只短爪自然抬起。保持画布位置与角色尺寸稳定。",
    triggers: [trigger("pixel-trigger-belly-double", { event: "double-click", enabled: true, region: bellyRegion })],
  }),
  transition({
    id: "pixel-lying-sit",
    label: "点击尾巴坐起来",
  ...
  transition({
    ...
    from: "state-pixel-sleeping",
    to: "state-pixel-lying",
    prompt: "长动作：Lottery 被双击后慢慢睁眼，抬头迷糊地看向用户，然后站起做一次睡眼惺忪的前腿伸展和礼貌鞠躬，打个小哈欠，最后慵懒地重新趴下并抬头。动作完整连续，不要加速。",
    triggers: [trigger("pixel-trigger-sleep-double", { event: "double-click", enabled: true, region: fullRegion })],
    durationSeconds: 8,
