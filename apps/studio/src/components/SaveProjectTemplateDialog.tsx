...
export function SaveProjectTemplateDialog({ project, onSave }: { project: CharacterProject; onSave: (name: string, description: string) => string | undefined }) {
  ...
  return (
    <Dialog.Root open={controller.open} onOpenChange={controller.setOpen}>
      <Button type="text" className="ghost-button" htmlType="button" disabled={project.logicalStates.length === 0} onClick={() => controller.setOpen(true)}><FileArrowDown size={15} />保存模板</Button>
      <Dialog.Portal>
        <Dialog.Overlay className="dialog-overlay" />
        <DialogContent className="dialog-content save-project-template-dialog">
          <div className="dialog-heading"><div><Dialog.Title>将当前项目保存为模板</Dialog.Title><Dialog.Description>复制状态机与交互规则，下一只宠物只需重新选择形象和风格。</Dialog.Description></div><Dialog.Close asChild><Button type="text" htmlType="button" className="icon-button" aria-label="关闭"><X size={18} /></Button></Dialog.Close></div>
          <form onSubmit={controller.submit}>
            ...
            <footer><Dialog.Close asChild><Button type="default" className="secondary-button" htmlType="button">取消</Button></Dialog.Close><Button type="primary" className="primary-button" htmlType="submit" disabled={!controller.name.trim()}><FileArrowDown size={15} weight="fill" />保存到全局模板库</Button></footer>
          </form>
        </DialogContent>
      </Dialog.Portal>
    </Dialog.Root>
