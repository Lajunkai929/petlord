  ...
  return (
    <main className="page-workspace review-workspace">
      <header className="review-page-heading">
        ...
        <div><Button type="default" className="secondary-button" htmlType="button" onClick={controller.chooseResponseFile}><FileArrowUp size={15} />导入反馈</Button><Button type="primary" className="primary-button" htmlType="button" disabled={Boolean(controller.exportingRoundId)} onClick={controller.createAndExportRound}>{controller.exportingRoundId ? <SpinnerGap className="spin" size={15} /> : <PaperPlaneTilt size={15} weight="fill" />}{controller.exportingRoundId ? `正在打包 ${controller.progress.completed}/${controller.progress.total}` : "新建评审"}</Button></div>
      </header>
      ...
      {controller.rounds.length === 0 ? (
        <section className="review-empty"><ChatCenteredText size={34} weight="thin" /><h2>还没有评审记录</h2><p>准备好状态或动画后创建第一轮评审。</p><Button type="primary" className="primary-button" htmlType="button" onClick={controller.createAndExportRound}>新建评审</Button></section>
      ) : (
                ...
                <PreviewableImageGroup><div className="review-item-grid">{round.items.map((item) => {
                  ...
                  return (
                    <article className={`is-${item.decision}`} key={item.id}>
                      <div className="review-item-media checkerboard">{artifact && (item.mediaKind === "video" ? <video src={artifact.uri} muted playsInline controls /> : <PreviewableImage nativePixel={artifact.nativePixel} src={artifact.uri} alt={item.label} />)}</div>
