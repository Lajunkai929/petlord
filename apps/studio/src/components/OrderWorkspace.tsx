...
import { ArrowClockwise, ArrowRight, ImageSquare } from "@phosphor-icons/react";
import { useInstalledPackageSync } from "../hooks/useInstalledPackageSync";
...
export function OrderWorkspace({
  ...
  const installed = useInstalledPackageSync();
  return (
    <main className="page-workspace project-library-workspace">
      <header className="project-library-header">
        ...
        <div className="project-library-actions"><span>{library.cards.length} 个项目 · {identities.length} 个形象</span><Button type="default" htmlType="button" disabled={installed.busy} onClick={() => { void installed.sync(); }}><ArrowClockwise size={15} />同步已安装宠物</Button><NewOrderDialog identities={identities} styles={styles} templates={templates} activeIdentityId={activeIdentityId} activeStyleProfileId={activeStyleProfileId} onCreate={onCreate} /></div>
      </header>
      {storageError && <p className="project-storage-error">项目未保存：{storageError}</p>}
      {installed.message && <p className="project-import-status" role="status">{installed.message}</p>}
      {installed.errors.map(error => <p className="project-storage-error" role="alert" key={error}>{error}</p>)}
      <PreviewableImageGroup><section className="project-library-grid" aria-label="项目列表">
        {library.cards.map((project) => (
          <article className={`project-library-card ${project.current ? "is-current" : ""}`} key={project.id}>
            <span className="project-library-thumb checkerboard">{project.thumbnail ? <PreviewableImage pixelated={project.pixelated} src={project.thumbnail} alt={`${project.characterName}项目预览`} /> : <ImageSquare size={24} weight="thin" />}</span>
            <button className="project-library-open" type="button" onClick={() => onOpenProject(project.id)} aria-label={`打开项目：${project.name}`}>
              <span className="project-library-main">
                ...
                {project.importLabel && <small>{project.importLabel}</small>}
