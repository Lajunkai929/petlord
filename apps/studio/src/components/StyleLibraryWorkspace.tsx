  ...
  return (
    <main className="page-workspace style-library-workspace">
      <header className="page-heading style-library-heading">
        ...
        <form className="style-create-form" onSubmit={controller.submitNewStyle}>
          <input value={controller.newStyleName} onChange={(event) => controller.setNewStyleName(event.target.value)} placeholder="新风格名称，例如 乡村像素伙伴" aria-label="新风格名称" />
          <button className="primary-button" type="submit" disabled={!controller.newStyleName.trim()}><Plus size={15} weight="bold" />新增风格</button>
        </form>
      </header>
      ...
      <div className="style-library-layout">
        ...
        {profile && <section className="style-profile-editor">
          <header className="style-profile-editor__header">
            ...
            <div>
              <button className="secondary-button" type="button" onClick={() => { studio.styleLibrary.applyProfile(profile.id); studio.setActiveArea("graph"); }}><Sparkle size={15} weight="fill" />应用到当前项目</button>
              <button className="primary-button" type="button" disabled={!lab.identity} onClick={lab.createStarterProject}><Plus size={15} weight="bold" />创建四状态项目</button>
            </div>
          </header>
          ...
          <div className="style-workbench">
            <section className="style-prompt-console">
              ...
              <article className="prompt-channel prompt-channel--image">
                <header><span><ImageSquare size={17} weight="fill" /><strong>图片通道 · Seedream</strong></span><small>造型、色块、五官、静态构图</small></header>
              ...
              <article className="prompt-channel prompt-channel--video">
                <header><span><FilmStrip size={17} weight="fill" /><strong>视频通道 · Seedance</strong></span><small>跨帧身份、线条、色块与动作稳定</small></header>
            ...
            <section className="style-experiment-panel">
              ...
              <div className="experiment-step">
                ...
                <div className="experiment-step__body">
                  ...
                  <label><span>调试形象</span><select value={lab.selectedIdentityId} onChange={(event) => lab.setSelectedIdentityId(event.target.value)}>{studio.identities.map((identity) => <option value={identity.id} key={identity.id}>{identity.name} · {identity.referenceArtifacts.length} 份参考</option>)}</select></label>
                  <div className="experiment-inline-fields">
                    ...
                    <label><span>候选数量</span><select value={lab.candidateCount} onChange={(event) => lab.setCandidateCount(Number(event.target.value))}>{[1, 2, 3, 4, 5].map((count) => <option value={count} key={count}>{count} 张</option>)}</select></label>
                  </div>
                  <label><span>本轮状态描述</span><textarea rows={3} value={lab.statePrompt} onChange={(event) => lab.setStatePrompt(event.target.value)} /></label>
                  <details className="experiment-prompt-preview"><summary>查看 Seedream 最终完整提示词</summary><pre>{lab.imagePromptPreview}</pre></details>
                  <div className="experiment-submit-row"><span><small>预计消耗</small><strong>¥{lab.imageEstimate?.maximumCny.toFixed(2) ?? "--"}</strong></span><button className="primary-button" type="button" disabled={!lab.identity?.referenceArtifacts.length || lab.imageBusy || lab.loading} onClick={lab.generateImageTest}>{lab.imageBusy || lab.loading ? <SpinnerGap className="spin" size={16} /> : <Sparkle size={16} weight="fill" />}{lab.imageBusy ? lab.imageStatusLabel : `生成 ${lab.candidateCount} 张图片`}</button></div>
                </div>
              </div>
              ...
              <div className="experiment-step">
                ...
                <div className="experiment-step__body">
                  ...
                  <details className="experiment-prompt-preview"><summary>查看 Seedance 最终完整提示词</summary><pre>{lab.videoPromptPreview}</pre></details>
                  <div className="experiment-submit-row"><span><small>预计消耗</small><strong>¥{lab.videoEstimate?.maximumCny.toFixed(2) ?? "--"}</strong></span><button className="primary-button" type="button" disabled={!lab.selectedImageUri || lab.videoBusy || lab.loading} onClick={lab.generateVideoTest}>{lab.videoBusy || lab.loading ? <SpinnerGap className="spin" size={16} /> : <Play size={16} weight="fill" />}{lab.videoBusy ? lab.videoStatusLabel : "生成测试视频"}</button></div>
                </div>
              </div>

              {lab.latestVideoJob?.result?.video && <div className="style-video-result"><header><span><Check size={14} weight="bold" />最近一次视频试验</span><small>实耗 ¥{lab.latestVideoJob.cost?.actualCny?.toFixed(2) ?? lab.latestVideoJob.cost?.estimatedMaxCny.toFixed(2) ?? "--"}</small></header><div className="checkerboard"><video src={lab.latestVideoJob.result.video.uri} controls loop muted playsInline /></div><details><summary>查看这次实际使用的提示词</summary><pre>{lab.latestVideoJob.assembledPrompt}</pre></details></div>}
            </section>
          </div>
