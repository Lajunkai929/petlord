      ...
      <PreviewableImageGroup><div className="transition-media-history__list">{versions.map((version) => {
        ...
        return (
          <article className={isActive ? "is-active" : ""} key={version.id}>
            <span className="transition-media-history__thumb checkerboard">{tail && <PreviewableImage nativePixel={tail.nativePixel} src={tail.uri} alt={`${version.label}结束帧`} />}</span>
