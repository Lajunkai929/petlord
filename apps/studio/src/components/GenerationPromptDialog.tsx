  ...
  return (
    <Dialog.Root>
      <Dialog.Trigger asChild>
        <button className={compact ? "prompt-history-button" : "prompt-preview-button"} type="button">{compact ? <Code size={12} /> : <Eye size={14} />}查看完整提示词</button>
      </Dialog.Trigger>
      <Dialog.Portal>
        <Dialog.Overlay className="dialog-overlay" />
        <Dialog.Content className="dialog-content generation-prompt-dialog">
          <div className="dialog-heading">
            <div><Dialog.Title>{title}</Dialog.Title><Dialog.Description>以下文本就是实际提交给 Seedance 的生成提示词。</Dialog.Description></div>
            <Dialog.Close className="icon-button" aria-label="关闭提示词"><X size={18} /></Dialog.Close>
          </div>
