import { SelectField, Input, Button } from "@petlord/ui";
  ...
  return (
    <section className="transition-playback-editor">
      ...
      <div className="transition-playback-editor__repeat">
        <label>重复方式
          <SelectField value={editor.playback.repeatMode} onChange={(event) => editor.updateRepeatMode(event.target.value as TransitionPlayback["repeatMode"])}>
      ...
      {props.isSelfTransition && editor.draftMode === "ping-pong"
        ? <form className="transition-playback-editor__ping-pong" onSubmit={editor.submitPingPong}>
            ...
            {props.sourceVideoUri && <div className="transition-playback-editor__preview">
              ...
              <Button type="default" htmlType="button" onClick={editor.previewSegment}><Play size={14} weight="fill" />{editor.previewingSegment ? "正在预览所选范围" : "预览所选范围"}</Button>
            </div>}
            ...
            <Button type="default" className="secondary-button" htmlType="submit" disabled={!props.hasVideo || props.busy || Boolean(editor.segmentError) || !editor.pingPongDraftUnsaved}>
