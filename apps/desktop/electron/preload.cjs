...
contextBridge.exposeInMainWorld("petLordDesktop", {
  ...
  showStudio: () => ipcRenderer.invoke("runtime:show-studio"),
  ...
  companionPlayback: input => ipcRenderer.send("runtime:companion-playback", input),
  setPetDragging: value => ipcRenderer.send("runtime:pet-dragging", value),
  performCompanionAction: action => ipcRenderer.invoke("runtime:perform-companion-action", action),
  listDesktopWaste: () => ipcRenderer.invoke("runtime:list-desktop-waste"),
  trashDesktopWaste: id => ipcRenderer.invoke("runtime:trash-desktop-waste", id),
  onCompanionAction: listener => { const h=(_e,value)=>listener(value); ipcRenderer.on("runtime:companion-action",h); return()=>ipcRenderer.removeListener("runtime:companion-action",h); },
  onCompanionFacing: listener => { const h=(_e,value)=>listener(value); ipcRenderer.on("runtime:companion-facing",h); return()=>ipcRenderer.removeListener("runtime:companion-facing",h); },
  onDesktopWaste: listener => { const h=(_e,value)=>listener(value); ipcRenderer.on("runtime:desktop-waste",h); return()=>ipcRenderer.removeListener("runtime:desktop-waste",h); },
  getCodexNotificationStatus: () => ipcRenderer.invoke("runtime:codex-notification-status"),
  installCodexNotifications: () => ipcRenderer.invoke("runtime:install-codex-notifications"),
  testCodexNotifications: () => ipcRenderer.invoke("runtime:test-codex-notifications"),
  ...
  editPackage: (key) => ipcRenderer.invoke("runtime:edit-package", key),
  ...
  getCodexDesignStatus: () => ipcRenderer.invoke("runtime:codex-design-status"),
  connectCodexDesign: () => ipcRenderer.invoke("runtime:connect-codex-design"),
