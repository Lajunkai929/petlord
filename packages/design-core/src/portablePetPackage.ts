import { characterProjectSchema, collectRuntimeMediaUris, mapRuntimeMediaUris, petPackageBundleSchema, type CharacterProject, type PetPackageBundle, type PetPackageManifest } from "@petlord/schema";
...
export async function createPortablePetBundle(
  ...
  sourceProject?: CharacterProject,
): Promise<PetPackageBundle> {
  const uris = [...new Set([...collectRuntimeMediaUris(manifest), ...(sourceProject?.artifacts.map(artifact => artifact.uri) ?? [])])];
  ...
  return buildPortablePetBundleV2(manifest, materialized, undefined, sourceProject);
}
...
export async function buildPortablePetBundleV2(
  ...
  const portableManifest = mapRuntimeMediaUris(manifest, uri => assetUris.get(uri) ?? uri);
  const portableSource = sourceProject ? characterProjectSchema.parse({...sourceProject, artifacts:sourceProject.artifacts.map(artifact => ({...artifact,uri:assetUris.get(artifact.uri) ?? artifact.uri}))}) : undefined;
  return petPackageBundleSchema.parse({
    format: "petlord-package",
    bundleVersion: 2,
    createdAt,
    manifest: portableManifest,
    ...(portableSource ? {sourceProject:portableSource} : {}),
    assets,
    integrity: {
      algorithm: "SHA-256",
      manifestSha256: await sha256(JSON.stringify(portableManifest)),
      ...(portableSource ? {sourceProjectSha256:await sha256(JSON.stringify(portableSource))} : {}),
      assets: assetHashes,
    },
  });
}
...
export function buildPortablePetBundle(
  ...
  const portableManifest = mapRuntimeMediaUris(manifest, uri => materialized.get(uri) ?? uri);
  return petPackageBundleSchema.parse({
    format: "petlord-package",
    bundleVersion: 1,
    createdAt,
