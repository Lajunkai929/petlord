...
const companionTransitions: TransitionBlueprint[] = [
  ...
  { id: "rest-belly", label: "趴着时单击翻肚皮", from: "rest", to: "play", prompt: (name) => `${name} 开心地从趴姿向侧面翻滚，再平稳翻到背上露出柔软肚皮，四只爪自然抬起；四肢长度、粗细和毛发必须严格继承当前形象参考。`, triggers: [trigger("belly-double", { event: "left-click", enabled: true, region: fullRegion })] },
  ...
  { id: "sit-rest", label: "坐着 30 秒后或单击后趴下", from: "idle", to: "rest", prompt: (name) => `${name} 安静坐了一会儿后放松身体，前爪向前伸，胸口和腹部缓慢落下，变成舒适趴姿。`, triggers: [
    trigger("sit-rest", { event: "inactivity", enabled: true, timerDurationMs: 30_000 }),
    trigger("sit-double-rest", { event: "left-click", enabled: true, region: fullRegion }),
  ] },
  { id: "sit-blink", label: "坐着呼吸、眨眼和动耳朵", from: "idle", to: "idle", prompt: (name) => `${name} 坐姿和身体位置保持稳定，胸腹持续轻微呼吸，自然眨眼两次，耳尖轻微响应，最后回到完全相同的坐姿。`, triggers: [trigger("sit-click", { event: "left-click", enabled: true, region: headRegion })], idleRule: { enabled: true, weight: 5, cooldownMs: 0 }, endFrameSource: "source-frame" },
  ...
  { id: "sleep-wake", label: "单击睡醒拉伸后趴下", from: "sleep", to: "rest", prompt: (name) => `${name} 被单击后慢慢睁眼，抬头迷糊地看向用户，站起做一次前腿伸展和礼貌鞠躬，打个小哈欠，最后慵懒地重新趴下。`, triggers: [trigger("sleep-double", { event: "left-click", enabled: true, region: fullRegion })], durationSeconds: 5, guidanceKeys: ["greet"] },
...
function mediaFreePointerGaze(pointerGaze: LogicalState["pointerGaze"]): LogicalState["pointerGaze"] {
  ...
  delete sanitized.nativeImageArtifactIds;
